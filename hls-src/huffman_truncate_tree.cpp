...
void truncate_tree(
  ...
  /* output */ ap_uint<SYMBOL_BITS> output_length_histogram_0[TREE_DEPTH],
  /* output */ ap_uint<SYMBOL_BITS> output_length_histogram_1[TREE_DEPTH]
  ) {
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=1 avg=1
      ...
      output_length_histogram_0[j] -= 1; // The node at level j is no longer a leaf.
      output_length_histogram_0[j+1] += 2; // Two new leaf nodes are attached at level j+1.
      output_length_histogram_0[i-1] += 1; // The leaf node at level i+1 gets attached here.
      output_length_histogram_0[i] -= 2; // Two leaf nodes have been lost from level i.
...
copy_output:
  for(int i = 0; i < TREE_DEPTH; i++) {
    ...
    assert(output_length_histogram_0[i] >= 0);
    assert(output_length_histogram_0[i] <= limit);
    limit *= 2;
  }
}
